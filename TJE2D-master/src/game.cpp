...
Game::Game(int window_width, int window_height, SDL_Window* window)
{
	...
	sprite.loadTGA("data/gotica_culona.tga"); //example to load an sprite
	sprite.scale(56, 72);
...
void Game::render(void)
{
	...
	//some new useful functions
		...
		//framebuffer.drawImage( sprite, player.position.x, player.position.y, Area((int (time * 4) % 4) * 14, player.orientation * 18,14,18) );	//draws only a part of an image
		...
		if (player.moving) {
			framebuffer.drawImage(sprite, player.position.x, 40 + player.position.y, Area((int(time * 4) % 4) * 14, player.orientation * 18, 14, 18));
		}
		else {
			framebuffer.drawImage(sprite, player.position.x, 40 + player.position.y, Area(0, player.orientation * 18, 14, 18));
		}
...
void Game::update(double seconds_elapsed)
{
	...
	player.orientation = stand;

	// Gravity
	if (player.position.y < groundLevel) {
		player.position.y += GRAVITY * seconds_elapsed * 10;
	}
	...
	if (player.isGround) {
		...
		{
			//player.position.y -= verticalVelocity * seconds_elapsed * speed;
			//verticalVelocity -= GRAVITY * seconds_elapsed;

			player.orientation = stand;
			player.moving = true;
			player.isGround = false;

			if (verticalVelocity >= 0.0f && player.position.y > -30) {
				verticalVelocity = 250.0f;
				player.position.y -= verticalVelocity * seconds_elapsed;
			}
	...
	{
		//player.position.y += seconds_elapsed * speed;
		//player.orientation = down;
		//player.moving = true;
	}
	if (Input::isKeyPressed(SDL_SCANCODE_RIGHT)) //if key right
	{
		player.position.x += seconds_elapsed * speed;
	...
	if (Input::wasKeyPressed(SDL_SCANCODE_SPACE)) //if key A was pressed
	{
		...
		if (verticalVelocity >= 0.0f && player.position.y > 40) {
